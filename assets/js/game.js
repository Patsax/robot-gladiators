...
var startGame = function () {
  ...
  for (var i = 0; i < enemyInfo.length; i++) {
    ...
    // if player robot is stil active, keep fighting
    if (playerInfo.health > 0) {
      ...
      // if player robot is still active after the fight and we're not at the last enemy in the array
    ...
    // if player robot is not active, break out of the loop and let endGame function run
    else {
...
var endGame = function () {
  ...
  if (playerInfo.money > highScore) {
    localStorage.setItem("higscore", playerInfo.money);
...
var shop = function () {
  ...
  var shopOptionPrompt = window.prompt(
    "Would you like to REFILL " + playerInfo.name + "'s health, UPGRADE " + playerInfo.name + "'s attack, or LEAVE the store? Please enter 1 to REFILL, 2 to UPGRADE, or 3 to LEAVE."
  );
